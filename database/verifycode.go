...
type VerifyCode struct {
	...
	GenerateDate *LocalTime `json:"generateDate" gorm:"column:generateDate;not null;type:datetime;default:CURRENT_TIMESTAMP"`
...
func GetTargetVerifyCode(email string, usage int) ([]VerifyCode, error) {
	var ret []VerifyCode
	err := db.Select("code").Where("email=? and usage_=? and generateDate>?", email, usage, time.Now().Add(time.Duration(-5*time.Minute))).Find(&ret).Error
	return ret, err
}
